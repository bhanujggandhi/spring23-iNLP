...
def sentence_perplexity(sentence: str, smoothing_type: str, infmode) -> float:
    ...
    while end <= len(sentence):
        ...
        probabs.append(c)
        perps.append((1 / c) ** (1 / (len(sentence))))
        log_p_sentence += np.log(c)
        start += 1
        end += 1
    if infmode:
        return np.prod(probabs)
    return np.prod(perps)
    # return np.exp(-log_p_sentence / (len(sentence)))
...
def get_perplexity(corpus: list[str], smoothing_type: str, mode: str) -> float:
    ...
    for i, sentence in enumerate(corpus):
        perp = sentence_perplexity(sentence, smoothing_type, False)
...
if not modelmode:
    while True:
        ...
        print(sentence_perplexity(inp, smtype, True), "\n")
